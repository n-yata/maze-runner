...
type GamePhase =
  ...
  | 'BOSS'        // ボス戦本編（縦シューティング: 左右移動のみ・レーザーは常に上方へ連射・弾を避けボスHPを削る。被弾はハート制で0でゲームオーバー）
...
パワーエサ取得時はプレイヤーに電磁バリアが付与される（レベル1: 6秒、上位レベルで短縮）。ゴーストは弱体化せず通常 AI のまま行動し、バリア接触で撃破される。
...
stateDiagram-v2
    ...
    BOSS --> BOSS: ボス弾被弾（ハート-1＋無敵。その場で続行）
    BOSS --> GAME_OVER: ハートを使い切る
...
| Inky | `#00FFFF`（シアン） |
| Clyde | `#FFB852`（オレンジ） |
| 電磁バリア | `#5FE6FF`（電磁シアンのグローリング、残量少で点滅） |
| 背景 | `#000000`（黒） |
